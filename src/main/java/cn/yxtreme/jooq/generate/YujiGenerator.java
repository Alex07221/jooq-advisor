...
public class YujiGenerator extends JavaGenerator {
    ...
    protected void printClassAnnotations(JavaWriter out, Definition definition, GeneratorStrategy.Mode mode) {
        if (isPojo) {
            ...
            out.println("@%s", out.ref("lombok.experimental.SuperBuilder"));
            out.println("@%s(chain = true)", out.ref("lombok.experimental.Accessors"));
        }
    ...
    private String ref(String clazzOrId) {
        return clazzOrId == null ? null : ref(List.of(clazzOrId)).get(0);
    }
    ...
    private void generatePojo0(Definition tableOrUDT, JavaWriter out) {
        ...
        int maxLength = 0;
        for (TypedElementDefinition<?> column : getTypedElements(tableOrUDT)) maxLength = Math.max(maxLength, out.ref(getJavaType(column.getType(resolver(out, Mode.POJO)), out, Mode.POJO)).length());
        ...
        for (TypedElementDefinition<?> column : getTypedElements(tableOrUDT)) {
            ...
            if (!BASE_TABLE_FIELD_NAME.contains(memberName)) {
                out.tab(1).println("private %s%s %s;", generateImmutablePojos() ? "final " : "", StringUtils.rightPad(out.ref(getJavaType(column.getType(resolver(out, Mode.POJO)), out, Mode.POJO)), maxLength), memberName);
            }
        }
        // Default constructor
        if (!generateImmutablePojos()) generatePojoDefaultConstructor(tableOrUDT, out);
        ...
        if (generateInterfaces() && !generateImmutablePojos() && tableOrUDT instanceof TableDefinition) {
    ...
    private List<? extends TypedElementDefinition<? extends Definition>> getTypedElements(Definition definition) {
        if (definition instanceof TableDefinition) return ((TableDefinition) definition).getColumns();
         else if (definition instanceof EmbeddableDefinition) return ((EmbeddableDefinition) definition).getColumns();
         else if (definition instanceof UDTDefinition) return ((UDTDefinition) definition).getAttributes();
         else if (definition instanceof RoutineDefinition) return ((RoutineDefinition) definition).getAllParameters();
